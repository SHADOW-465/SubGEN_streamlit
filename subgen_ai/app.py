...
def render_sidebar() -> None:
    ...
    with st.sidebar:
        ...
        # Auto-detect ESP32 ports once per session
        if not st.session_state["esp32_ports"]:
            detected = find_esp32_port()
            if detected:
                st.session_state["esp32_ports"] = [detected]
                st.session_state["port"] = detected
                st.session_state["hw_status"] = "connected"
            else:
                st.session_state["esp32_ports"] = []
        ...
        st.session_state["port"] = None if selected_port == "Software only" else selected_port

        # Status indicator
        hw_status = st.session_state["hw_status"]
        if st.session_state["port"] is not None:
            st.markdown("🟢 **Connected** — Hardware MFCC active")
        elif hw_status == "error":
            st.markdown("🔴 **Not found** — check USB cable")
        else:
            st.markdown("🔵 **Software mode** — SW-MFCC fallback")

        # Test connection button
        if st.button("🔌 Test Connection", use_container_width=True):
            _test_esp32_connection()
...
def _test_esp32_connection() -> None:
    """Send 100 silence samples to ESP32 and display latency."""
    import numpy as np, time
    port = st.session_state.get("port")
    if port is None:
        st.sidebar.info("ℹ Running in software-only mode — no hardware to test.")
        return
    dummy = np.zeros(100, dtype=np.float32)
    t0 = time.time()
    try:
        result = get_fingerprint(dummy, 16000, esp32_port=port)
        latency_ms = (time.time() - t0) * 1000
        if result.get("ok"):
            st.sidebar.success(f"✅ ESP32 responded in {latency_ms:.0f} ms")
            st.session_state["hw_status"] = "connected"
        else:
            st.sidebar.error(f"❌ ESP32 error: {result.get('error', 'unknown')}")
            st.session_state["hw_status"] = "error"
    except Exception as exc:
